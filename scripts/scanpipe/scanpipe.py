...
class Cloud:
    ...
    # ---- Talk ----
    def nachrichten(self, limit=50):
        url = f"{self.basis}/ocs/v2.php/apps/spreed/api/v1/chat/{self.raum}"
        r = self.s.get(url, params={"lookIntoFuture": 0, "limit": limit}, timeout=30)
        r.raise_for_status()
        return list(reversed(r.json()["ocs"]["data"]))   # älteste zuerst

    def sende(self, text, antwortAuf=None):
        ...
        if self.konfig.get("sendeUeberOpenclaw", True):
            befehl = [self.konfig.get("openclaw", "openclaw"), "message", "send",
                      "--channel", "nextcloud-talk", "--target", self.raum,
                      "--message", text]
            if antwortAuf:
            ...
            try:
                ...
                if p.returncode == 0:
                    return self._letzte_bot_nachricht()
    ...
    def _letzte_bot_nachricht(self):
        ...
        for _ in range(4):
            try:
                for m in self.nachrichten(limit=6)[::-1]:
    ...
    def reagiere(self, nachrichtId, emoji):
        url = (f"{self.basis}/ocs/v2.php/apps/spreed/api/v1/reaction/"
               f"{self.raum}/{nachrichtId}")
        try:
...
def korrektur_pruefen(cloud, konfig, zustand, nachricht):
    ...
    if not treffer:
        cloud.sende("Diese Domäne kenne ich nicht. Möglich sind: " +
    ...
    if ok:
        ...
        eintrag["pfad"] = neu
        cloud.sende(f"✅ Verschoben nach **{treffer}**\n`{neu}`", antwortAuf=nachricht["id"])
        log("Korrektur:", alt, "->", neu)
    else:
        cloud.sende(f"Das Verschieben ging nicht (HTTP {code}).", antwortAuf=nachricht["id"])
    return True


# ---------------------------------------------------------------- Hauptlauf

def verarbeite(cloud, konfig, zustand, nachricht, arbeitsverzeichnis: Path):
    ...
    with tempfile.TemporaryDirectory(prefix="scanpipe-") as tmp:
        ...
        try:
            ...
            cloud.sende(f"⚠️ Konnte `{name}` nicht laden: {e}", antwortAuf=nachricht["id"])
            return
        ...
        if mime.startswith("image/"):
            ...
            if not shutil.which("pdftoppm"):
                cloud.sende("⚠️ PDF-Verarbeitung braucht `pdftoppm`, das fehlt noch.",
            ...
            if not seiten:
                cloud.sende("⚠️ Aus dem PDF liess sich keine Seite lesen.",
                            antwortAuf=nachricht["id"])
                return
            bild = seiten[0]
        else:
            cloud.sende(f"⚠️ `{mime}` kann ich nicht lesen. Schick ein Foto oder ein PDF.",
                        antwortAuf=nachricht["id"])
            return
        ...
        if fehler:
            ...
            cloud.sende(f"⚠️ Erkennung fehlgeschlagen: {fehler[:200]}",
                        antwortAuf=nachricht["id"])
            return

        if daten.get("domaene") == "kein_dokument":
            cloud.sende("🤔 Das sieht nicht nach einem Dokument aus. Ich lasse es liegen.",
                        antwortAuf=nachricht["id"])
            return

        ziel, fehler = einsortieren(cloud, konfig, original, daten, pfad)
        if fehler:
            cloud.sende(f"⚠️ {fehler}", antwortAuf=nachricht["id"])
            return

        berichtId = cloud.sende(bericht(daten, ziel, cloud), antwortAuf=nachricht["id"])
        cloud.reagiere(nachricht["id"], "✅")
        if berichtId:
            zustand.setdefault("berichte", {})[str(berichtId)] = {
                "pfad": ziel, "quelle": nid
            }
        log("Abgelegt:", ziel)
...
def main():
    ...
    try:
        nachrichten = cloud.nachrichten(limit=konfig.get("holeAnzahl", 50))
    except requests.RequestException as e:
        log("Chat nicht erreichbar:", e)
        return 1

    letzte = int(zustand.get("letzteId", 0))
    neu = [m for m in nachrichten if int(m.get("id", 0)) > letzte]
    if not neu:
        return 0

    log(f"{len(neu)} neue Nachricht(en) seit {letzte}")
    for m in neu:
        try:
            ...
            if besitzer and m.get("actorId") != besitzer:
                continue
            if korrektur_pruefen(cloud, konfig, zustand, m):
                continue
            mp = m.get("messageParameters") or {}
            if mp.get("file"):
                verarbeite(cloud, konfig, zustand, m, arbeitsverzeichnis)
        except Exception as e:                      # eine kaputte Nachricht darf nicht alles stoppen
            log("Fehler bei Nachricht", m.get("id"), ":", repr(e))
        finally:
            zustand["letzteId"] = max(int(zustand.get("letzteId", 0)), int(m.get("id", 0)))
